...
class NoteController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        $validated = $request->validate([
            ...
            'averages.*' => 'numeric|min:0|max:20', // Assure que chaque moyenne est entre 0 et 20
        ]);
        ...
        try {
            foreach ($request->averages as $studentId => $noteValue) {
                $noteValue = $noteValue ?? 0; // Par défaut à 0 si null

                // Vérifier si une note existe déjà pour cet étudiant, cette matière et ce semestre
                ...
                // Si une note existe déjà, la sauter
                if ($existingNote) {
                    continue;
                }
    ...
    public function generateBulletinsPDF(Request $request)
    {
        ...
        $bulletins = $bulletins->sortByDesc('moyenne_generale')->values()->map(function ($bulletin, $index) {
            $sex = $bulletin['student']->sex; // Récupérer le sexe de l'élève
            $bulletin['rank'] = $this->format($index + 1, $sex); // Formatage du rang en fonction du sexe
            return $bulletin;
        });
        ...
        if ($request->semester_hidden == 2) {
            $bulletins = $bulletins->sortByDesc('moyenne_annuelle')->values()->map(function ($bulletin, $index) {
                $sex = $bulletin['student']->sex; // Récupérer le sexe de l'élève
                $bulletin['annual_rank'] = $this->format($index + 1, $sex); // Formatage du rang annuel en fonction du sexe
                return $bulletin;
            });
        }
    ...
    protected function format($rank, $sex) {
        if ($rank == 1) {
            return $sex == 'F' ? $rank . 'ère' : $rank . 'er';
        } else {
            return $rank . 'ème';
        }
    }
    ...
    public function getNotes(Request $request)
    {
        ...
        $request->validate([
            ...
            'semester' => 'required|in:1,2', // Assurez-vous que le semestre correspond à vos règles
        ]);
        ...
        try {
            ...
            // Récupérer les étudiants de la classe et de l'année spécifiées
            $students = Student::whereHas('recordings', function ($query) use ($yearId, $classroomId) {
                $query->where('year_id', $yearId)
                    ->where('classroom_id', $classroomId)
                    ->where('school_id', auth()->user()->school_id); // Filtrer par l'école de l'utilisateur connecté
            })->get();

            // Transformer les données des étudiants et leurs notes
            $notesData = $students->map(function ($student) use ($yearId, $classroomId, $subjectId, $semester) {
                ...
                return [
                    ...
                    'average' => $note ? $note->note : null,
                    'note_exists' => $note ? true : false, // Indique si la note existe déjà
                ];
            });
            ...
            // Journaliser les données récupérées
